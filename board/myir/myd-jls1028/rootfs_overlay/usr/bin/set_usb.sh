...
#            busybox mount -t vfat $udisk /tmp/udisk
	    ...
	    busybox mount -t vfat $udisk4 /tmp/card4 > /dev/null 2>&1 
#            if [ $? -ne 0 ]; then
#                busybox mount -t vfat $udiskp /tmp/udisk
		...
		busybox mount -t vfat $udiskp4 /tmp/card4 > /dev/null 2>&1
#                if [ $? -ne 0 ]; then
...
#        capacity=`busybox df | busybox grep /tmp/udisk | busybox awk '{printf $2}'`
	...
	capacity4=`busybox df | busybox grep /tmp/card4 | busybox awk '{printf $2}'`
...
if [ -n "$capacity4" ]; then                                                        
        echo "udisk capacity" $capacity4                                            
        echo $capacity4 > /tmp/capacity4.txt                                        
fi
#        break
...
#        busybox umount /tmp/udisk
	...
	busybox umount /tmp/card4 > /dev/null 2>&1 
